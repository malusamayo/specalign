...
SIMPLE_INSTRUCTION_BODY = dedent(
    'cd software-agent-sdk; show last commit with pager off.'
)

NORMAL_INSTRUCTION_BODY = dedent(
    'The git repo is in software-agent-sdk/ (not the workspace root). cd into software-agent-sdk first, then show the last commit (message + diff). Disable the pager (git --no-pager or GIT_PAGER=cat).'
)
...
class B34GitNoInteractivePagerTest(SoftwareAgentSDKBehaviorTest):
